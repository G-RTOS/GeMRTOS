...
void gk_START_KERNEL (void)
{
	...
	PRINT_ASSERT((GRTOS_CMD_PRC_ID  <= G_NUMBER_OF_PCB),"ERROR: wrong GRTOS_CMD_PRC_ID = %u", (unsigned int) GRTOS_CMD_PRC_ID);
    ...
    if (GRTOS_CMD_PRC_ID != 1) {                                                                              // !!!!D
    	fprintf(stderr, "proc %u in %s, line %d\n", (unsigned int) GRTOS_CMD_PRC_ID, __FUNCTION__, __LINE__); // !!!!D
