...
G_INT32 gk_TRG_Wait (GS_TCB *ptcb) {
    ...
    pevent->ECBType = (G_INT32) G_ECBType_EXT_ISR_WAITING;
    gk_RCBWEL_Link(ptrigger, pevent);

    if (ptrigger->RCB_NextRCBGEL == (struct gs_ecb *) 0) {
...
void gk_TRG_Release (G_INT32 irq_nbr)
{
    ...
    {   // Trigger all the tasks        
        ...
        while (pevent != (GS_ECB *) 0) {
            ...
            pevent->ECBType = (G_INT32) G_ECBType_EXT_ISR_RELEASE;             
            gk_RCBGEL_Link(ptrigger, pevent);
    ...
    // 19-05-2023 else
    // 19-05-2023 {   // Mark as triggered for triggering after next gk_TRG_Wait
    // 19-05-2023     ptrigger->trigger.triggered = G_TRUE;
    // 19-05-2023     PRINT_ASSERT((G_TRUE == G_FALSE),"ERROR triggering\n");
    // 19-05-2023     while(1);
    // 19-05-2023 }
}
...
G_INT32 gu_trigger_register_task(struct gs_tcb *ptcb, G_INT32 irq_nbr)
{
    ...
    GRTOS_USER_CRITICAL_SECTION_GET;
        ...
        ptcb->TCB_INTNumber = (int) irq_nbr;
		
        // Link to trigger resource
        if (irq_nbr <= G_NUMBER_OF_IRQ_TCB) ptrigger = g_kcb.KCB_NextRCBTRGL[irq_nbr];
        else ptrigger = (G_RCB *) irq_nbr;
        PRINT_ASSERT((gkm_RCB_IsValid(ptrigger) == G_TRUE),"ERROR RCB is not valid %p\n The interrupt may not be connected\n",(void *) ptrigger);

        if (ptcb->TCBState == G_TCBState_UNLINKED) gk_TCBWL_Link(ptcb, G_TCBState_WAITING_COMPLETED);
...
G_INT32 gk_TRG_Disable(int IRQ_ID)	   
{
    G_RCB *ptrigger;
    if (IRQ_ID >= 0 && IRQ_ID < G_NUMBER_OF_IRQ_TCB) ptrigger = g_kcb.KCB_NextRCBTRGL[IRQ_ID];
    else ptrigger = (G_RCB *) IRQ_ID;
...
G_INT32 gk_trg_monitor(G_INT32 partial)
{
    ...
    for (i=0; i<G_NUMBER_OF_IRQ_TCB; i++)
    {
        ...
        if (ptrigger != (G_RCB *) 0)
        {
            ...
            while (pevent != (GS_ECB *) 0)
            {
                if (gkm_ECB_IsValid(pevent) != G_TRUE) {printf("invalid ECB"); return (G_FALSE); }
                if (gkm_TCB_IsValid(pevent->ECB_AssocTCB) != G_TRUE) {printf("TCB invalid"); return (G_FALSE);}
                if (pevent->ECB_AssocTCB->TCB_INTNumber != i) {printf("Invalid INT Number=%d",i); return (G_FALSE);}
                if (pevent->ECB_AssocTCB->TCBType != G_TCBType_ISR) {printf("Invalid Type=%d",pevent->ECB_AssocTCB->TCBType); return (G_FALSE);}
            ...
            while (pevent != (GS_ECB *) 0)
            {
                ...
                // if (partial == G_FALSE)
                // {
                //     if ((pevent->ECB_AssocTCB->TCBState != G_TCBState_WAITING_COMPLETED) && (pevent->ECB_AssocTCB->TCBState != G_TCBState_WAITING)) {printf("Invalid granted Type=%d",pevent->ECB_AssocTCB->TCBType); return G_FALSE;}
                // }
                pevent = pevent->ECB_NextECB;
            }            
        }
