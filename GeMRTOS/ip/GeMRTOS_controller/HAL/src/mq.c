...
int gu_queue_receive(G_RCB *prcb, void * buffer_msg, G_INT32 buffer_length) {
    ...
    GRTOS_USER_CRITICAL_SECTION_GET;
        ...
        // 19-05-2023 prcb = (G_RCB *) pevent->ECB_AssocRCB;
        ...
        pevent->ECB_RRDS->queue_buffer.rrds_consumer.buffer_length = buffer_length;
        ...
        if (pevent->ECB_RRDS->queue_buffer.rrds_consumer.pproducer != (GS_ECB  *) 0) {
            ...
            if (pevent1->ECB_RRDS->queue_buffer.rrds_producer.deliveries >= pevent1->ECB_RRDS->queue_buffer.rrds_producer.consumers) {
                ...
                pevent1->ECBType = G_ECBType_MESSAGE_PRODUCE_DELIVERED;
        ...
        while (pevent1 != (GS_ECB *) 0) {
            ...
            if (pevent1->ECB_RRDS->queue_buffer.rrds_producer.MQ_msg_seq > pevent->ECB_RRDS->queue_buffer.rrds_consumer.MQ_msg_seq) {
                ...
                memcpy(pevent->ECB_RRDS->queue_buffer.rrds_consumer.buffer_msg, pevent1->ECB_RRDS->queue_buffer.rrds_producer.pmsg, pevent1->ECB_RRDS->queue_buffer.rrds_producer.msg_length);
 ...
 */
int gu_queue_send(G_RCB *prcb, void *pmsg, int msg_length, gt_time timeout) {
    ...
    GRTOS_USER_CRITICAL_SECTION_GET;
        ...
        PRINT_ASSERT((gkm_RCB_IsValid(prcb) == G_TRUE),"ERROR RCB is not valid\n");
        PRINT_ASSERT((prcb->RCB_NextRCBGEL != (struct gs_ecb *) 0),"ERROR No cumsemer registered\n");
        if (prcb->RCB_NextRCBGEL == (struct gs_ecb *) 0) { // No consumer in queue, return 0 messages deliverred
        ...
        pevent->ECBType = G_ECBType_MESSAGE_PRODUCE_WAIT;
        ...
        while (pevent1 != (GS_ECB *) 0) {
            ...
            if (pevent1->ECBType == G_ECBType_MESSAGE_CONSUME_WAIT) {
                pevent1->ECBType = G_ECBType_QUEUE_CONSUME;
                pevent1->ECB_RRDS->queue_buffer.rrds_consumer.pproducer = pevent;
                
                PRINT_ASSERT((pevent->ECB_RRDS->queue_buffer.rrds_producer.msg_length < pevent1->ECB_RRDS->queue_buffer.rrds_consumer.buffer_length),"ERROR string too long\n");
                
                memcpy(pevent1->ECB_RRDS->queue_buffer.rrds_consumer.buffer_msg, pevent->ECB_RRDS->queue_buffer.rrds_producer.pmsg, pevent->ECB_RRDS->queue_buffer.rrds_producer.msg_length);
        ...
        gk_KERNEL_TASK_SUSPEND_CURRENT();    //Switch tasks and wait to deliver or timeout
        // #################################################
      
        if (pevent->ECBType == G_ECBType_MESSAGE_PRODUCE_DELIVERED) 
        {
...
int gu_queue_printf(G_RCB *prcb, char *format, ...)
{
    ...
    // 29-02-2024 GRTOS_USER_CRITICAL_SECTION_GET;
    ...
    const unsigned int BUF_LEN = 255;
    char x[BUF_LEN + 2];
	x[BUF_LEN + 1] = 0;
	...
	// 01-03-2024 GRTOS_USER_CRITICAL_SECTION_RELEASE;
	
#ifdef __niosX_arch__
    int i;
    // fprintf(fpuart[GRTOS_CMD_PRC_ID-1], "%s", x);
	i = gu_queue_send(prcb, (void *) x, (G_INT32) (strlen(x) + 1), (gt_time) 0);
#else
    printf("%s", x);
#endif
...
GS_ECB *gu_queue_subscribe(GS_TCB  *ptcb, G_RCB  *presource)
{
	...
	pevent = gk_RCBGEL_Link(presource, (GS_ECB *) 0); // Create and link an ECB to RCBGEL
	PRINT_ASSERT((gkm_ECB_IsValid(pevent) == G_TRUE),"ERROR ECB is not valid\n");
