...
G_INT32 gk_ECBAEL_Link(GS_ECB * const pevent1, GS_ECB * const pevent2)
{
    ...
    //if ((GS_ECB *) *((char *) pevent1 + (G_INT32) (&((GS_ECB *) NULL)->ECB_NextECBAEL)) == (GS_ECB *) 0) 
    {
 ...
 *  \relates Signal
 */ 
void  gk_ECBASL_Link(GS_ECB * const pevent, GS_SCB * const psignal)
{
    GK_SAMPLE_FUNCTION_BEGIN(4)

    GS_SCB *psignal2;

    if ((pevent->ECB_NextECBASL == (struct gs_scb *) 0) || (pevent->ECB_NextECBASL->SCBType > psignal->SCBType))
    {
        // it is the only SCB associated to pevent
    	psignal->SCB_NextSCB   = (struct gs_scb *) pevent->ECB_NextECBASL;
    ...
    else 
    {
        ...
        while ((psignal2->SCB_NextSCB != (struct gs_scb *) 0) && (psignal2->SCB_NextSCB->SCBType < psignal->SCBType))
        {
            PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_ECBASL_Link\n");
            psignal2 = psignal2->SCB_NextSCB;
        }
        psignal->SCB_NextSCB = psignal2->SCB_NextSCB;
        psignal2->SCB_NextSCB = psignal;
    }
    psignal->SCBState = G_SCBState_IN_ECB;
    GK_SAMPLE_FUNCTION_END(4)
}
...
G_INT32  gk_ECBASL_Unlink(GS_ECB *pevent, GS_SCB *psignal)
{
    ...
    {
    	psignal2 = pevent->ECB_NextECBASL;
    	while ((psignal2->SCB_NextSCB != (struct gs_scb *) 0) && (psignal2->SCB_NextSCB != psignal))
    	{
    		psignal2 = psignal2->SCB_NextSCB;
    	}
        PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE), "ERROR signal not in ECBASL");

		psignal2->SCB_NextSCB = psignal->SCB_NextSCB;
		psignal->SCBState = G_SCBState_UNLINKED;
		retorno = G_TRUE;
...
G_INT32  gk_ECBTL_Link (GS_ECB *pevent)
{
	...
	register GS_ECB *pevent2;
    
    PRINT_ASSERT((gkm_ECB_IsValid(g_kcb.KCB_NextECBTL) == G_TRUE),"ERROR ECB time list is empty\n");
    PRINT_ASSERT((pevent->ECBState == GS_ECBState_UNLINKED),"ERROR ECBState= %d\n", (int) pevent->ECBState);
	
	OC_priority.i64 = pevent->ECBValue.i64;
	pevent2 = g_kcb.KCB_NextECBTL;
	
    if (pevent2->ECBValue.i64 > OC_priority.i64)
		{/* Event inserted is the next occurence   */
		set_pointed_field(pevent, ECB_NextECB, pevent2);
		set_pointed_field(pevent, ECB_PrevECB, (struct gs_ecb *) 0);
		set_pointed_field(pevent2, ECB_PrevECB, (struct gs_ecb *) pevent);
		...
		while (pevent2->ECBValue.i64 <= OC_priority.i64){
			pevent2 = get_pointed_field(pevent2, ECB_NextECB);
		}        
		set_pointed_field(pevent, ECB_NextECB, (struct gs_ecb *) pevent2); /* Insert the task in the linked list */
		set_pointed_field(pevent, ECB_PrevECB, pevent2->ECB_PrevECB);
		set_pointed_field(pevent2->ECB_PrevECB, ECB_NextECB, (struct gs_ecb *) pevent);
		set_pointed_field(pevent2, ECB_PrevECB, (struct gs_ecb *) pevent);
	}
...
G_INT32 gk_ECBTL_Unlink(GS_ECB *pevent)
{
	...
	GS_ECB *pevent1 = g_kcb.KCB_NextECBTL;
    
    PRINT_ASSERT((gkm_ECB_IsValid(pevent) == G_TRUE),"ERROR ECB not valid\n");
    PRINT_ASSERT((pevent->ECBState == GS_ECBState_WAITING_TIME),"ERROR ECBState= %d\n", (int) pevent->ECBState);    

	/* Remove from the time linked list */
	if (pevent1 == (struct gs_ecb *) pevent)
	{
		g_kcb.KCB_NextECBTL = get_pointed_field(pevent, ECB_NextECB);   
		pevent1 = g_kcb.KCB_NextECBTL;
		set_pointed_field(pevent1, ECB_PrevECB, (struct gs_ecb *) 0);
		GRTOS_CMD_NXT_OCC_TM_EVN_SET(pevent1->ECBValue.i64);
	} else
	{
        set_pointed_field(get_pointed_field(pevent, ECB_PrevECB), ECB_NextECB, get_pointed_field(pevent, ECB_NextECB));
        set_pointed_field(get_pointed_field(pevent, ECB_NextECB), ECB_PrevECB, get_pointed_field(pevent, ECB_PrevECB));
	}
	/* Set the pointer to unlinked */
	set_pointed_field(pevent, ECB_PrevECB, (struct gs_ecb *) 0);
	set_pointed_field(pevent, ECB_NextECB, (struct gs_ecb *) 0);
	set_pointed_field(pevent, ECBState, GS_ECBState_UNLINKED);
...
{
    GK_SAMPLE_FUNCTION_BEGIN(10)

    GS_SCB *psignal2;

    if ((g_kcb.KCB_NextKCBASL == (struct gs_scb *) 0) || (g_kcb.KCB_NextKCBASL->SCBType > psignal->SCBType))
    {
        // it is the only SCB associated to pevent
...
G_INT32  gk_KCBASL_Unlink(GS_SCB *psignal)
{
    ...
    else
    {
    	...
    	{
    		psignal2 = psignal2->SCB_NextSCB;
			PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_KCBASL_Unlink\n");
    	}
    	if (psignal2->SCB_NextSCB == psignal)
    	{
    		psignal2->SCB_NextSCB = psignal->SCB_NextSCB;
    		psignal->SCBState = G_SCBState_UNLINKED;
    		retorno = G_TRUE;
...
G_INT32 gk_LCBL_UnLink(GS_LCB *plcb)
{
	...
	if (g_kcb.KCB_NextLCBL == plcb)	{/* Remove as first element  */
    ...
    else {/* It is not the firstLCB in the list */
...
G_INT32 gk_LCBFPL_Unlink(G_INT32 processorID)
{
    ...
    if (get_pointed_field(plcb, LCB_NextLCBFPL) == ppcb) { /* It is the first element */
        set_pointed_field(plcb, LCB_NextLCBFPL, get_pointed_field(ppcb, PCB_NextPCB));
        if (get_pointed_field(plcb, LCB_NextLCBFPL) != (struct gs_pcb *) 0) set_pointed_field(plcb->LCB_NextLCBFPL, PCB_PrevPCB, (struct gs_pcb *) 0);
    }
    else 
    {
        set_pointed_field(ppcb->PCB_PrevPCB, PCB_NextPCB, get_pointed_field(ppcb, PCB_NextPCB));
        if (get_pointed_field(ppcb, PCB_NextPCB) != (struct gs_pcb *) 0) set_pointed_field(ppcb->PCB_NextPCB,PCB_PrevPCB, get_pointed_field(ppcb, PCB_PrevPCB));
    }

	set_pointed_field(ppcb, PCBState, GS_PCBState_RUNNING);
    set_pointed_field(ppcb, PCB_NextPCB, (struct gs_pcb *) 0);
    set_pointed_field(ppcb, PCB_PrevPCB, (struct gs_pcb *) 0);
...
{
    GK_SAMPLE_FUNCTION_BEGIN(16)
    GS_SCB *psignal2;

    if ((presource->RCB_NextRCBASL == (struct gs_scb *) 0) || (presource->RCB_NextRCBASL->SCBType > psignal->SCBType))
    {
        // it is the only SCB associated to pevent
    ...
    {
		// Sort by priority
		psignal2 = presource->RCB_NextRCBASL;
		while ((psignal2->SCB_NextSCB != (struct gs_scb *) 0) && (psignal2->SCB_NextSCB->SCBType < psignal->SCBType))
		{
			PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_ECBASL_Link\n");
			psignal2 = psignal2->SCB_NextSCB;
...
GS_ECB *gk_RCBGEL_Link(G_RCB *presource, GS_ECB *pevent)
{
	...
	if (get_pointed_field(pevent, ECB_RRDS) != (struct gs_rrds *) 0) set_pointed_field(pevent, ECBValue.i64, pevent->ECB_RRDS->RRDSGrantedPriority.i64);
	else set_pointed_field(pevent, ECBValue.i64, (G_INT64) G_LOWEST_PRIORITY);

	/* Get the priority of the Event of highest priority */
	pevent1 = get_pointed_field(presource, RCB_NextRCBGEL);
	if ((pevent1 == (struct gs_ecb *) 0) || (pevent1->ECBValue.i64 > pevent->ECBValue.i64))
	{
	    set_pointed_field(pevent, ECB_NextECB, (struct gs_ecb *) pevent1);
	    set_pointed_field(pevent, ECB_PrevECB, (struct gs_ecb *)0);
		if (pevent1 != (struct gs_ecb *) 0) set_pointed_field(pevent1, ECB_PrevECB,(struct gs_ecb *) pevent);
	    set_pointed_field(presource, RCB_NextRCBGEL, (struct gs_ecb *) pevent);
	}
	else 
	{
		while ((get_pointed_field(pevent1, ECB_NextECB) != (struct gs_ecb *) 0) && (pevent1->ECB_NextECB->ECBValue.i64 <= pevent->ECBValue.i64))
		{
			pevent1 = get_pointed_field(pevent1, ECB_NextECB);
			PRINT_ASSERT((gkm_ECB_IsValid(pevent1) == G_TRUE),"ERROR ECB1 not valid\n");
		}
		set_pointed_field(pevent, ECB_NextECB, (struct gs_ecb *) get_pointed_field(pevent1, ECB_NextECB));
		set_pointed_field(pevent, ECB_PrevECB, pevent1);
		if (get_pointed_field(pevent1, ECB_NextECB) != (struct gs_ecb *) 0) set_pointed_field(get_pointed_field(pevent1,ECB_NextECB), ECB_PrevECB, pevent);
		set_pointed_field(pevent1, ECB_NextECB, (struct gs_ecb *) pevent);
...
void  gk_RCBGEL_Unlink(GS_ECB *pevent)
{
	...
	if (get_pointed_field(presource, RCB_NextRCBGEL) == pevent) {
		set_pointed_field(presource, RCB_NextRCBGEL, pevent->ECB_NextECB);
	} else {
        PRINT_ASSERT((pevent->ECB_PrevECB != (struct gs_ecb *) 0),"ERROR in gk_RCBGEL_Unlink");
        set_pointed_field(get_pointed_field(pevent, ECB_PrevECB), ECB_NextECB, get_pointed_field(pevent, ECB_NextECB));
	}
    if (get_pointed_field(pevent, ECB_NextECB) != (struct gs_ecb *) 0) set_pointed_field(get_pointed_field(pevent, ECB_NextECB), ECB_PrevECB, pevent->ECB_PrevECB);
...
GS_ECB *gk_RCBWEL_Link(G_RCB *presource, GS_ECB *pevent)
{
	...
	register GS_ECB *pevent1;

	// Get event if it is NULL
	if (pevent == (struct gs_ecb *) 0){
		pevent = gk_ECB_GetFree();
		if (pevent != (struct gs_ecb *) 0) set_pointed_field(pevent, ECB_RRDS, (struct gs_rrds *) 0);
	}
	if (pevent == (struct gs_ecb *) 0) G_DEBUG_WHILEFOREVER;


	// Set the priority of the event
	if (get_pointed_field(pevent, ECB_RRDS) != (struct gs_rrds *) 0) set_pointed_field(pevent, ECBValue.i64, pevent->ECB_RRDS->RRDSWaitingPriority.i64);
	else set_pointed_field(pevent, ECBValue.i64, presource->RCBPriority.i64);

	/* Get the priority of the Event of highest priority */
	pevent1 = get_pointed_field(presource, RCB_NextRCBWEL);
	
	OC_priority.i64 = pevent->ECBValue.i64;

	if ((pevent1 == (struct gs_ecb *) 0) || (pevent1->ECBValue.i64 > OC_priority.i64))
	{
	    set_pointed_field(pevent, ECB_NextECB, (struct gs_ecb *) presource->RCB_NextRCBWEL);
	    set_pointed_field(pevent, ECB_PrevECB, (struct gs_ecb *) 0);
		if (pevent1 != (struct gs_ecb *) 0) set_pointed_field(presource->RCB_NextRCBWEL, ECB_PrevECB, (struct gs_ecb *) pevent);
	    set_pointed_field(presource, RCB_NextRCBWEL, (struct gs_ecb *) pevent);
	}
	else
	{
		while ((get_pointed_field(pevent1, ECB_NextECB) != (struct gs_ecb *) 0) && (pevent1->ECB_NextECB->ECBValue.i64 <= OC_priority.i64)) {
			pevent1 = get_pointed_field(pevent1, ECB_NextECB);
		}
		set_pointed_field(pevent, ECB_NextECB, pevent1->ECB_NextECB);
		set_pointed_field(pevent, ECB_PrevECB, pevent1);
		if (get_pointed_field(pevent1, ECB_NextECB) != (struct gs_ecb *) 0) set_pointed_field(pevent1->ECB_NextECB,ECB_PrevECB, pevent);
		set_pointed_field(pevent1, ECB_NextECB, pevent);
	}
	set_pointed_field(pevent, ECBState, GS_ECBState_WAITING_RESOURCE);
	set_pointed_field(pevent, ECB_AssocRCB, (struct g_rcb *) presource);
...
void gk_RCBWEL_Unlink(GS_ECB *pevent)
{
	...
	/* Remove from the resource linked list */
	if (get_pointed_field(presource, RCB_NextRCBWEL) == pevent)
	{
		set_pointed_field(presource, RCB_NextRCBWEL, get_pointed_field(pevent, ECB_NextECB));
		if (get_pointed_field(presource, RCB_NextRCBWEL) !=  (struct gs_ecb *) 0) 
			set_pointed_field(get_pointed_field(presource,RCB_NextRCBWEL), ECB_PrevECB,(struct gs_ecb *) 0);
	} else
	{
        PRINT_ASSERT((pevent->ECB_PrevECB != (struct gs_ecb *) 0), "ERROR in gk_RCBWEL_Unlink");
        set_pointed_field(get_pointed_field(pevent, ECB_PrevECB), ECB_NextECB, get_pointed_field(pevent,ECB_NextECB));
        if (get_pointed_field(pevent, ECB_NextECB) != (struct gs_ecb *) 0) {
            set_pointed_field(get_pointed_field(pevent, ECB_NextECB), ECB_PrevECB, get_pointed_field(pevent, ECB_PrevECB));
        }
	}
	/* Set the pointer to unlinked */
...
G_INT32 gk_TCBAEL_Unlink(GS_ECB *pevent)
{
    ...
    {
        if (get_pointed_field(get_pointed_field(pevent, ECB_AssocTCB), TCB_NextTCBAEL) == pevent) {
            set_pointed_field(get_pointed_field(pevent, ECB_AssocTCB), TCB_NextTCBAEL, get_pointed_field(pevent, ECB_NextTCBAEL));
        }
        else {
            set_pointed_field(get_pointed_field(pevent, ECB_PrevTCBAEL), ECB_NextTCBAEL, get_pointed_field(pevent, ECB_NextTCBAEL));
        }
        if (get_pointed_field(pevent, ECB_NextTCBAEL) != (struct gs_ecb *) 0) set_pointed_field(get_pointed_field(pevent, ECB_NextTCBAEL), ECB_PrevTCBAEL, get_pointed_field(pevent, ECB_PrevTCBAEL));

        /* Set the event unlink from task list */
        set_pointed_field(pevent, ECB_PrevTCBAEL, (struct gs_ecb *) 0);
        set_pointed_field(pevent, ECB_NextTCBAEL, (struct gs_ecb *) 0);
...
{
    GK_SAMPLE_FUNCTION_BEGIN(10044)
    G_INT32 priority = get_pointed_field(psignal, SCBType);
    GS_SCB *psignal2;

    if ((get_pointed_field(ptcb, TCB_NextTCBASL) == (struct gs_scb *) 0) || (get_pointed_field(get_pointed_field(ptcb, TCB_NextTCBASL), SCBType) > priority))
    {
        // it is the only SCB associated to pevent
    	set_pointed_field(psignal, SCB_NextSCB, (GS_SCB *) get_pointed_field(ptcb, TCB_NextTCBASL));
    	set_pointed_field(ptcb, TCB_NextTCBASL, (struct gs_scb *) psignal);
    }
	else
	{
		// Sort by priotiy
		psignal2 = (GS_SCB *) get_pointed_field(ptcb, TCB_NextTCBASL);
		while ((get_pointed_field(psignal2, SCB_NextSCB) != (struct gs_scb *) 0) && (get_pointed_field(get_pointed_field(psignal2, SCB_NextSCB), SCBType) <= priority))
		{
			PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_TCBASL_Link\n");
			psignal2 = get_pointed_field(psignal2, SCB_NextSCB);
		}
		PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_TCBASL_Link2\n");
...
G_INT32  gk_TCBASL_Unlink(GS_TCB *ptcb, GS_SCB *psignal)
{
	...
	if (ptcb->TCB_NextTCBASL != (struct gs_scb *) 0)
	{
		...
		{
			psignal1 = (GS_SCB *) ptcb->TCB_NextTCBASL;
			while ((psignal1->SCB_NextSCB != (struct gs_scb *) 0) && (psignal1->SCB_NextSCB != psignal))
			{
				PRINT_ASSERT((gkm_SCB_IsValid(psignal1) == G_TRUE),"ERROR psignal 2\n");
				psignal1 = psignal1->SCB_NextSCB;
			}
			PRINT_ASSERT((psignal1 == psignal),"ERROR psignal 3\n");
			psignal1->SCB_NextSCB = psignal->SCB_NextSCB;
		}
		psignal->SCBState = G_SCBState_UNLINKED;
	}
...
G_INT32  gk_TCBPSL_Unlink(GS_TCB *ptcb, GS_SCB *psignal)
{
	...
	if (get_pointed_field(ptcb, TCB_NextTCBPSL) != (struct gs_scb *) 0)
	{
		if (get_pointed_field(ptcb, TCB_NextTCBPSL) == psignal)
		{
			set_pointed_field(ptcb, TCB_NextTCBPSL, get_pointed_field(psignal, SCB_NextSCB));
		}
		else
		{
			psignal1 = (GS_SCB *) get_pointed_field(ptcb, TCB_NextTCBPSL);
			while ((get_pointed_field(psignal1, SCB_NextSCB) != (struct gs_scb *) 0) && (get_pointed_field(psignal1, SCB_NextSCB) != psignal))
			{
				psignal1 = get_pointed_field(psignal1, SCB_NextSCB);
				PRINT_ASSERT((gkm_SCB_IsValid(psignal1) == G_TRUE),"ERROR psignal in gk_TCBPSL_Unlink\n");  
			}
			PRINT_ASSERT((psignal1->SCB_NextSCB == psignal),"ERROR psignal in gk_TCBPSL_Unlink 2\n");
			set_pointed_field(psignal1, SCB_NextSCB,  get_pointed_field(psignal, SCB_NextSCB));
		}
		set_pointed_field(psignal, SCBState, G_SCBState_UNLINKED);
    }
...
G_INT32 gk_TCBRDYL_Link(GS_TCB * const ptcb)
{
	...
	OC_priority.i64 = ptcb->TCBCurrentPriority;
	
	if (get_pointed_field(ptcb, TCBType) != G_TCBType_IDLE) /* Idle task are not inserted in Ready Lists */
	{
		if ((get_pointed_field(readylist,LCB_NextTCBRDYL) == (struct gs_tcb *) 0) || (readylist->LCB_NextTCBRDYL->TCBCurrentPriority > OC_priority.i64))
		{// It is the first TCB to insert
			set_pointed_field(ptcb, TCB_NextTCB, (struct gs_tcb *) get_pointed_field(readylist,LCB_NextTCBRDYL));
			set_pointed_field(ptcb, TCB_PrevTCB, (struct gs_tcb *) 0);
			if (get_pointed_field(readylist, LCB_NextTCBRDYL) != (struct gs_tcb *) 0) set_pointed_field(get_pointed_field(ptcb, TCB_NextTCB), TCB_PrevTCB, (struct gs_tcb *) ptcb);
			set_pointed_field(readylist, LCB_NextTCBRDYL, (struct gs_tcb *) ptcb); 
		}
		else {
			ptcb1 = get_pointed_field(readylist, LCB_NextTCBRDYL); 
			while ((get_pointed_field(ptcb1, TCB_NextTCB) != (struct gs_tcb *) 0) && (ptcb1->TCB_NextTCB->TCBCurrentPriority <= OC_priority.i64))
			{
				ptcb1 = (GS_TCB *) get_pointed_field(ptcb1, TCB_NextTCB);
			}
			
			set_pointed_field(ptcb, TCB_NextTCB, get_pointed_field(ptcb1, TCB_NextTCB));
			if (get_pointed_field(ptcb1, TCB_NextTCB) != (struct gs_tcb *) 0) set_pointed_field(get_pointed_field(ptcb1, TCB_NextTCB), TCB_PrevTCB, ptcb);
			set_pointed_field(ptcb1, TCB_NextTCB, ptcb);
			set_pointed_field(ptcb, TCB_PrevTCB, (struct gs_tcb *) ptcb1);
		}
...
G_INT32 gk_TCBRDYL_Unlink(GS_TCB *ptcb)
{
    ...
    {
		if (get_pointed_field(readylist, LCB_NextTCBRDYL) == ptcb) /* It is the first ready task */
		{
			set_pointed_field(readylist, LCB_NextTCBRDYL, (struct gs_tcb *) get_pointed_field(ptcb,TCB_NextTCB)); 
			if (get_pointed_field(ptcb, TCB_NextTCB) != (GS_TCB *)0) set_pointed_field(get_pointed_field(ptcb, TCB_NextTCB),TCB_PrevTCB, (struct gs_tcb *) 0); 
		}
		else
		{ /* It is not the first one */
			if (get_pointed_field(ptcb, TCB_NextTCB) != (GS_TCB *)0) set_pointed_field(get_pointed_field(ptcb, TCB_NextTCB), TCB_PrevTCB, ptcb->TCB_PrevTCB);
            PRINT_ASSERT((ptcb->TCB_PrevTCB != (GS_TCB *)0), "ERROR gk_TCBRDYL_Unlink");
			set_pointed_field(get_pointed_field(ptcb, TCB_PrevTCB), TCB_NextTCB, get_pointed_field(ptcb, TCB_NextTCB)); 
		}
    }

	set_pointed_field(ptcb, TCBState, G_TCBState_UNLINKED);
    set_pointed_field(ptcb, TCB_PrevTCB, (GS_TCB *) 0);
    set_pointed_field(ptcb, TCB_NextTCB, (GS_TCB *) 0);
...
G_INT32  gk_TCBRUNL_Link(GS_TCB *ptcb)
{
	...
	// 15-05-2023 ptcb->TCB_AssocPCB = (G_INT32) GRTOS_CMD_PRC_ID; 

	ptcb1 = get_pointed_field(RunList, LCB_NextTCBRUNL);
	if ((ptcb1 == (struct gs_tcb *) 0) || (OC_priority.i64 > ptcb1->TCBCurrentPriority))
	{
		/* It is the first element in the running list */
		set_pointed_field(ptcb, TCB_NextTCB, (struct gs_tcb *) ptcb1);
		set_pointed_field(ptcb, TCB_PrevTCB, (struct gs_tcb *) 0);
		if (ptcb1 != (struct gs_tcb *) 0)
			set_pointed_field(ptcb1, TCB_PrevTCB, (struct gs_tcb *) ptcb);
		set_pointed_field(RunList, LCB_NextTCBRUNL, (struct gs_tcb *) ptcb); 
	}
	else
	{
		PRINT_ASSERT((gkm_TCB_IsValid(ptcb1) == G_TRUE),"ERROR TCB not valid ptcb= %p\n", (void *) ptcb1);
		...
		set_pointed_field(ptcb, TCB_NextTCB, get_pointed_field(ptcb1, TCB_NextTCB));
		set_pointed_field(ptcb, TCB_PrevTCB, (struct gs_tcb *) ptcb1);
		if (get_pointed_field(ptcb1, TCB_NextTCB) != (struct gs_tcb *) 0)
			set_pointed_field(get_pointed_field(ptcb1, TCB_NextTCB), TCB_PrevTCB, ptcb);
		set_pointed_field(ptcb1, TCB_NextTCB, ptcb);
	}
...
G_INT32  gk_TCBRUNL_Unlink(GS_TCB *ptcb)
{
    ...
    if (get_pointed_field(RunList,LCB_NextTCBRUNL) == ptcb){ // checks if it is the first TCP in RUNNING LIST
    	set_pointed_field(RunList, LCB_NextTCBRUNL, (GS_TCB *) get_pointed_field(ptcb,TCB_NextTCB)); 
    	if (get_pointed_field(RunList, LCB_NextTCBRUNL) != (struct gs_tcb *) 0) /* It is not the only element */
    	{
    		set_pointed_field(RunList->LCB_NextTCBRUNL, TCB_PrevTCB, (struct gs_tcb *) 0);
    	}
    }
    else
    {
    	/* It is not the first element in the list */
		set_pointed_field(ptcb->TCB_PrevTCB, TCB_NextTCB, ptcb->TCB_NextTCB); 
		if (get_pointed_field(ptcb, TCB_NextTCB) != (GS_TCB *)0) set_pointed_field(ptcb->TCB_NextTCB, TCB_PrevTCB, ptcb->TCB_PrevTCB); 
    }
...
G_INT32 gk_TCBWL_Unlink(GS_TCB *ptcb)
{
    ...
    if (g_kcb.KCB_NextTCBWL == ptcb) {
        g_kcb.KCB_NextTCBWL = (GS_TCB *) get_pointed_field(ptcb, TCB_NextTCB);
    }
    else set_pointed_field(ptcb->TCB_PrevTCB, TCB_NextTCB, get_pointed_field(ptcb,TCB_NextTCB));
    if (get_pointed_field(ptcb,TCB_NextTCB) != (struct gs_tcb *) 0) set_pointed_field(get_pointed_field(ptcb,TCB_NextTCB), TCB_PrevTCB,get_pointed_field(ptcb, TCB_PrevTCB));
...
G_INT32 gk_TASK_IS_BLOCKED(GS_TCB *ptcb)
{
	...
	G_INT32 return_int; PRINT_DEBUG_LINE
...
G_INT32 gk_RRDSFL_Link(GS_RRDS *prrds)
{
	...
	// Remove SCB from ASL
	while (get_pointed_field(prrds, RRDS_NextSCB) != (struct gs_scb *) 0)
	{
		psignal1 = get_pointed_field(prrds, RRDS_NextSCB);
 ...
 *  \relates Signal
 */ 
G_INT32 gk_RRDSASL_Link(GS_RRDS *prrds, GS_SCB *psignal)
{
    GK_SAMPLE_FUNCTION_BEGIN(49)
    G_INT32 priority = psignal->SCBType;
    GS_SCB *psignal2;

    if ((prrds->RRDS_NextSCB == (struct gs_scb *) 0) || (prrds->RRDS_NextSCB->SCBType > priority))
    {
        // it is the only SCB associated to pevent
    	psignal->SCB_NextSCB = (struct gs_scb *) prrds->RRDS_NextSCB;
    	prrds->RRDS_NextSCB  = (struct gs_scb *) psignal;
    }
	else
	{
		// Sort by priotiy
		psignal2 = prrds->RRDS_NextSCB;
		while ((psignal2->SCB_NextSCB != (struct gs_scb *) 0) && (psignal2->SCB_NextSCB->SCBType <= priority))
		{
			psignal2 = psignal2->SCB_NextSCB;
		}
		PRINT_ASSERT((gkm_SCB_IsValid(psignal2) == G_TRUE),"ERROR psignal in gk_RRDSASL_Link\n");
		psignal->SCB_NextSCB  = psignal2->SCB_NextSCB;
		psignal2->SCB_NextSCB = (struct gs_scb *) psignal;
    }
	
	psignal->SCBState = G_SCBState_IN_TCB;
    GK_SAMPLE_FUNCTION_END(49)
    return(G_TRUE);
}
 ...
 *  \relates RRSD
 */ 
G_INT32  gk_RRDSASL_UnLink(GS_RRDS *prrds, GS_SCB *psignal)
{
    GK_SAMPLE_FUNCTION_BEGIN(50)
	GS_SCB *psignal1;
	...
	if (prrds->RRDS_NextSCB != (struct gs_scb *) 0)
	{
		...
		else
		{
			...
			PRINT_ASSERT((gkm_SCB_IsValid(psignal1) == G_TRUE),"ERROR psignal in gk_RRDSASL_UnLink\n");
			psignal1->SCB_NextSCB = psignal->SCB_NextSCB;
		}
		psignal->SCBState = G_SCBState_UNLINKED;
	}

    PRINT_ASSERT((psignal->SCBState == G_SCBState_UNLINKED),"ERROR psignal->SCBState= %d\n",psignal->SCBState);

    GK_SAMPLE_FUNCTION_END(50)
    return(G_TRUE);
}
...
GS_TCB *gk_PCB_GetCurrentTCB(void)
{
	...
	// 		fprintf(stderr, "In %s, %d, proc= %x, CRITICAL = %x, MASK= %x\n",__FUNCTION__,__LINE__, GRTOS_CMD_PRC_ID,IORD(GRTOS_DRIVER_GRTOS_BASE, ADDR_PRC_CRTCL_SECTION_START),PRC_MASK[GRTOS_CMD_PRC_ID-1]);
...
GS_TCB *gk_PCB_GetNextTCB(void)
{
	...
	GS_PCB *ppcb = (GS_PCB *) &g_kcb.G_PCBTbl[GRTOS_CMD_PRC_ID -1];
    register GS_PCBAssocLCB *ppcbalcb = ppcb->PCB_AssocLCB;
...
G_INT32 gk_SetLowestProcessor(void)
{
    ...
    register int i = 1;
    int lowest_pcb = 0;
    // G_INT64 priority = g_kcb.G_PCBTbl[lowest_pcb].PCB_EXECTCB->TCBCurrentPriority;
    ...
    if (get_pointed_field(ppcb, PCBState) == GS_PCBState_FREE)
    {
        OC_priority.i64 = OC_priority.i64 | (G_INT64) G_IDLE_PRIORITY_MASK;
    }
    ...
    while (i < G_NUMBER_OF_PCB) {
		...
		{
			OC_priority2.i64 = ptcb->TCBCurrentPriority;
			if (get_pointed_field(ppcb, PCBState) == GS_PCBState_FREE)
			{
				OC_priority2.i64 = OC_priority2.i64 | (G_INT64) G_IDLE_PRIORITY_MASK;
			}
			if (OC_priority2.i64 > OC_priority.i64)
			{
				OC_priority.i64 = OC_priority2.i64;
				lowest_pcb = i;
			}
		}
        i++;
 ...
 *  \relates Task
 */
G_INT32 gk_TASK_INHERENCE_PRIORITY_SET(GS_TCB *ptcb)
{
    GK_SAMPLE_FUNCTION_BEGIN(57)
	GS_ECB *pevent;
	G_INT64 priority;
	G_INT64 priority2;
	G_RCB *presource;
...
{   
    ...
    GS_PCB *ppcb;
    GS_TCB *ptcb;
    register GS_LCB *readylist = g_kcb.KCB_NextLCBL;
        ...
        {        
            ...
            if ((ppcb != (struct gs_pcb *) 0))
            {
                if (get_pointed_field(readylist, LCBExclusion) > get_pointed_field(readylist, LCBCurrentRunning))
                {
                    ...
                    // while (ppcb != (struct gs_pcb *) 0)
                    // {
                        PRINT_ASSERT((ppcb->PCBState == GS_PCBState_FREE), "ERROR PCB state is not valid=%d\n",(int) ppcb->PCBState);
                        
                        ptcb = get_pointed_field(ppcb, PCB_EXECTCB);
                        // if (GRTOS_CMD_PRC_ID != get_pointed_field(ppcb, PCBID)) {
                        ...
                        // }
					// 	break;
                    //     ppcb = get_pointed_field(ppcb, PCB_NextPCB);
                    // }
                }
            }
            else
            {
                ...
                if (ptcb != (struct gs_tcb *) 0)
                {
                    ...
                    if (ptcb->TCBCurrentPriority > readylist->LCB_NextTCBRDYL->TCBCurrentPriority)
                    {
						/* Ready priority is greater then processor assigned is interrupted */
						gk_TCB_Unlink(ptcb);           // 24-05-2023 !!!!!!!!!
						gk_TCBRDYL_Link(ptcb);             // 24-05-2023 !!!!!!!!!
						// ptcb = get_pointed_field(readylist, LCB_NextTCBRUNL); // 24-05-2023 !!!!!!!!! 
                    }
                }
            }
        }
...
//  
//     GK_SAMPLE_FUNCTION_BEGIN(61)
//  	G_DEBUG_VERBOSE
//  
//  if G_DEBUG_WHILEFOREVER_ENABLE == 1
//  	if (gkm_ECB_IsValid(pecb) != G_TRUE) G_DEBUG_WHILEFOREVER;
//  endif
//  
...
{
    GK_SAMPLE_FUNCTION_BEGIN(62)
	GS_SCB *psignal = (GS_SCB *) 0;
	GS_SCB *psignal1 = (GS_SCB *) ptcb->TCB_NextTCBPSL;
 ...
 *  \sa RRDS
 */
GS_SCB *gk_RRDSASL_GetSCB(GS_RRDS *prrds, G_INT32 SignalType)
{
    GK_SAMPLE_FUNCTION_BEGIN(63)
...
{
    GK_SAMPLE_FUNCTION_BEGIN(10041)
	GS_SCB *psignal = (GS_SCB *) 0;
	GS_SCB *psignal1 = (GS_SCB *) get_pointed_field(ptcb, TCB_NextTCBASL);

    while (psignal1 != (struct gs_scb *) 0)
    {
    	if (get_pointed_field(psignal1, SCBType) == SignalType)
    	{
    		psignal = psignal1;
    		psignal1 = (struct gs_scb *) 0;
    	}
 ...
 *  \sa Signal
 */ 
GS_SCB *gk_ECBASL_GetSCB(GS_ECB *pecb, G_INT32 SignalType)
{
    GK_SAMPLE_FUNCTION_BEGIN(66)
	GS_SCB *psignal = (struct gs_scb *) 0;
 ...
 *  \relates Signal
 */ 
GS_SCB *gk_RCBASL_GetSCB(G_RCB *prcb, G_INT32 SignalType)
{
    GK_SAMPLE_FUNCTION_BEGIN(67)
	GS_SCB *psignal = (struct gs_scb *) 0;
...
{
	return ((G_INT32) (*((volatile unsigned int *) GRTOS_DRIVER_GRTOS_PROCESSOR_BASE1 + 1)));
}
