...
{
	G_INT64 temp_aux;
	G_INT64 hours_64 = (G_INT64) hours;
	G_INT64 minutes_64 = (G_INT64) minutes;
	G_INT64 seconds_64 = (G_INT64) seconds;
	G_INT64 ms_64 = (G_INT64) ms;

	temp_aux = ms_64 * (G_INT64) G_TICKS_PER_MSECOND;
	temp_aux = temp_aux + seconds_64 * (G_INT64) G_TICKS_PER_SECOND;
	temp_aux = temp_aux + minutes_64 * (G_INT64) (G_TICKS_PER_SECOND * G_SECONDS_IN_MINUTE);
	temp_aux = temp_aux + hours_64 * (G_INT64) G_TICKS_PER_SECOND * (G_INT64) G_SECONDS_IN_HOUR;
	return (temp_aux);
...
void *gu_GetTask(void *TaskCode, void *p_arg, char *format, ...)
{
    ...
    GRTOS_USER_CRITICAL_SECTION_GET;
		...
		if (ptcb != (struct gs_tcb *) 0) {
			...
			// 05-07-2024 ptcb->TCB_Abort_w_Deadline = G_TRUE;   /* True when task is aborted when deadline, false otherwise */
...
G_INT32 gk_TASK_PRIORITY_SET_CALLBACK(GS_TCB *ptcb, G_INT32 task_state)
{
    ...
    switch (lcbtype)
    {
        case GS_LCBType_EDF:
            ...
            // 19-06-2024 else
            // 19-06-2024 {
            // 19-06-2024     switch (task_state)
            // 19-06-2024     {
            // 19-06-2024         case G_TCBState_READY:   /* Insert in Ready list */
            // 19-06-2024             set_pointed_field(ptcb, TCBCurrentPriority, ptcb->TCBReadyPriority);
            // 19-06-2024             break;
            // 19-06-2024         case G_TCBState_RUNNING: /* Insert in Running list */
            // 19-06-2024             set_pointed_field(ptcb, TCBCurrentPriority, ptcb->TCBRunPriority);
            // 19-06-2024             break;
            // 19-06-2024         default:
            // 19-06-2024 			G_DEBUG_WHILEFOREVER;
            // 19-06-2024 			break;
            // 19-06-2024     }                
            // 19-06-2024 }
            break;
