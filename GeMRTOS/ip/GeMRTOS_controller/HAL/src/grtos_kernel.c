...
// const unsigned int G_PROCESSOR_MASK[] = {0x0, 0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 0x80, 0x100, 0x200, 0x400, 0x800, 0x1000, 0x2000, 0x4000, 0x8000, 0x10000, 0x20000, 0x40000, 0x80000, 0x100000, 0x200000, 0x400000, 0x800000, 0x1000000, 0x2000000, 0x4000000, 0x8000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0xFFFFFFFF};
	...
	const struct gemrtos_jtag_uart_info G_UART_INFO[] = GRTOS_DRIVER_JTAG_ARRAY;
#endif
...
void gk_KERNEL_TASK_START (void)
{
	...
	// 08-07-2024 fprintf(stderr,"%s, GRTOS_IRQ_INPUT_STATUS=%x, event=%x", __FUNCTION__, (unsigned int) GRTOS_IRQ_INPUT_STATUS, (unsigned int) event_code);
	if (event_code != 0)
	{
        switch (event_code) {
            case EVN_CODE_TIMED:
...
// CREATE_SUSPENDED
   //  R_PRC_RST = 1;
...
#ifdef __niosX_arch__
	GU_FPRINTF(stderr,"[ MESSAGE ] _REENT %p\n", (void *) _REENT);
	// 15-07-2024 GU_FPRINTF(stderr,"[ MESSAGE ] &_impure_ptr %p\n", (void *) &_impure_ptr);
	// 15-07-2024 GU_FPRINTF(stderr,"[ MESSAGE ] _impure_ptr %p\n", (void *) _impure_ptr);
	// 15-07-2024 GU_FPRINTF(stderr,"[ MESSAGE ] size of _impure_ptr %u\n", (unsigned int) sizeof(struct _reent));
	GU_FPRINTF(stderr,"[ MESSAGE ] size of G_UART_INFO %u\n", (unsigned int) sizeof(G_UART_INFO));
	GU_FPRINTF(stderr,"[ MESSAGE ] size of struct gemrtos_jtag_uart_info %u\n", (unsigned int) sizeof(struct gemrtos_jtag_uart_info));
	GU_FPRINTF(stderr,"[ MESSAGE ] number of struct gemrtos_jtag_uart_info %u\n", (unsigned int) (sizeof(G_UART_INFO) / sizeof(struct gemrtos_jtag_uart_info)));
	for (int i=0; i < (unsigned int) (sizeof(G_UART_INFO) / sizeof(struct gemrtos_jtag_uart_info)) ;i++) {
		if ((void *) ALT_STDOUT_BASE == G_UART_INFO[i].JTAG_UART_BASE) {
			GU_FPRINTF(stderr,"[ MESSAGE ] STDOUT_BASE = %u\n", (unsigned int) G_UART_INFO[i].JTAG_UART_BASE);
			GU_FPRINTF(stderr,"[ MESSAGE ] STDOUT_IRQ = %u\n", (unsigned int) G_UART_INFO[i].JTAG_UART_IRQ);
			GU_FPRINTF(stderr,"[ MESSAGE ] STDOUT_JTAG_UART_IRQ_INTERRUPT_CONTROLLER_ID = %u\n", (unsigned int) G_UART_INFO[i].JTAG_UART_IRQ_INTERRUPT_CONTROLLER_ID);
			GU_FPRINTF(stderr,"[ MESSAGE ] STDOUT_JTAG_UART_NAME = %s\n", (char *) G_UART_INFO[i].JTAG_UART_NAME);
			GU_FPRINTF(stderr,"[ MESSAGE ] STDOUT_JTAG_UART_READ_DEPTH = %u\n", (unsigned int) G_UART_INFO[i].JTAG_UART_READ_DEPTH);
		}
	}
	...
	// 02-07-2024 /* List all device names registered in the HAL */ // 26-06-2024
	// 02-07-2024 alt_llist *llist = &alt_dev_list;
	// 02-07-2024 alt_dev* next = (alt_dev*) llist->next;
	// 02-07-2024 while (next != (alt_dev*) llist)
	// 02-07-2024 {	
	// 02-07-2024 	GU_FPRINTF(stderr,"[ MESSAGE ] list %s\n", (char *) next->name);
	// 02-07-2024 	next = (alt_dev*) next->llist.next;
	// 02-07-2024 }
	// 02-07-2024 
	// 02-07-2024 /* Show a list of the state of the interrupts */ // 26-06-2024
	// 02-07-2024 for (unsigned int proc_irq = 0; proc_irq <= 31; proc_irq ++) {
	// 02-07-2024 	GU_FPRINTF(stderr,"[ MESSAGE ] interrupt %u, state=%d, pending=%u\n", (unsigned int) proc_irq, alt_ic_irq_enabled (0, proc_irq), (unsigned int) alt_irq_pending());
	// 02-07-2024 }
}
...
void gk_START_KERNEL (void)
{
    ...
    if (GRTOS_CMD_PRC_ID  == (int) 1) {
		...
		// IOWR_GRTOS_PROC_REENT(&_impure_data);
#ifdef __niosX_arch__
...
#ifndef __niosX_arch__
    if (GRTOS_CMD_PRC_ID != 1) {
    	fprintf(stderr, "proc %u in %s, line %d\n", (unsigned int) GRTOS_CMD_PRC_ID, __FUNCTION__, __LINE__);
    ...
    // 18-07-2024 do{
    // 18-07-2024 	do {
    // 18-07-2024         GRTOS_MTX_RSV_SET;
    // 18-07-2024         GRTOS_CMD_HALT_PROCESSOR;
    // 18-07-2024 		NO_OP
    // 18-07-2024     } while (GRTOS_CMD_PRC_ID != GRTOS_MTX_PRC_GRANTED);
    // 18-07-2024 	GU_FPRINTF(stderr,"[ MESSAGE ] GRANTED %u\n", GRTOS_MTX_PRC_GRANTED);
    // 18-07-2024 	GU_FPRINTF(stderr,"[ MESSAGE ] CPUID   %u\n", GRTOS_CMD_PRC_ID);
    // 18-07-2024 	PRINT_ASSERT((GRTOS_MTX_PRC_GRANTED == GRTOS_CMD_PRC_ID),"ERROR getting critical section= %d, proc=%d, in %s, %d\n", (int) GRTOS_MTX_PRC_GRANTED, (int) GRTOS_CMD_PRC_ID, __FUNCTION__, __LINE__);
    // 18-07-2024 	gk_CRITICAL_SECTION_HIGHEST_PRIORITY_PROCESSOR_CALLBACK();
    // 18-07-2024 }while(0);
	...
	GRTOS_ENABLE_GeMRTOS_IRQ;            /// Enable Processor Interrupt from gemrtos controller (IRQ 0)
	// 09-07-2024 alt_ic_irq_enable ((alt_u32) GRTOS_DRIVER_GRTOS_S_PROCESSOR1_IRQ_INTERRUPT_CONTROLLER_ID, (alt_u32) GRTOS_DRIVER_GRTOS_S_PROCESSOR1_IRQ);
...
void gk_CheckASM(unsigned int idata)
{
	...
	GU_FPRINTF(stderr,"INFO: MSTATUS = %x\n", data);
	NIOSV_READ_MTVEC(data);
	GU_FPRINTF(stderr,"INFO: MTVEC = %x\n", data);
	NIOSV_READ_MCAUSE(data);
	GU_FPRINTF(stderr,"INFO: MCAUSE = %x\n", data);
	NIOSV_READ_MIE(data);
	GU_FPRINTF(stderr,"INFO: MIE = %x\n", data);
	NIOSV_READ_MIP(data);
	GU_FPRINTF(stderr,"INFO: MIP = %x\n", data);
	NIOSV_READ_MEPC(data);
	GU_FPRINTF(stderr,"INFO: MEPC = %x\n", data);
	GRTOS_READ_SP(data);
	GU_FPRINTF(stderr,"INFO: SP = %p\n", (void *) data);
#endif
}
...
void gk_restore(void)
{
    ...
    printf("autorestore proc= %u, RST_REG= %u\n", (unsigned int) GRTOS_CMD_PRC_ID, (unsigned int) IORD(GRTOS_DRIVER_GRTOS_BASE, ADDR_RST_CLR));
    while(1);
}
...
void gk_WAIT_IDLE_TASK(void)
{
	...
	// 02-07-2024 volatile register int *idlestate = &g_kcb.G_PCBTbl[GRTOS_CMD_PRC_ID -1].GRTOS_PROCESSOR_BASE[ADDR_REG_0];
...
void gk_CODE_IDLE_TASK(void* pdata)
{
	...
	while(1)
    {
		...
		// 12-06-2024 GRTOS_CMD_HALT_PROCESSOR; // Put the procesor in HALT mode
		GRTOS_CMD_IDLE_PROCESSOR;

		PRINT_ASSERT((GRTOS_MTX_PRC_GRANTED != GRTOS_CMD_PRC_ID), "ERROR in IDLE task MTX= %u, PROC=%u\n", (unsigned int) GRTOS_MTX_PRC_GRANTED, (unsigned int) GRTOS_CMD_PRC_ID);
    }
}
