...
{
	// fprintf(stderr,"ERROR gkm_XCB_NotValid");
    return(G_TRUE);
}
...
G_INT32 gkm_ECBType_Valid(enum ecbtype ecbtype)
{
    ...
    if (ecbtype == G_ECBType_MESSAGE_PRODUCE_DELIVERED) {return (G_TRUE);}
...
GS_ECB *gkm_ECB_list_ROOT(GS_ECB *pevent)
{
    ...
    if (pevent->ECBType == G_ECBType_MESSAGE_PRODUCE_DELIVERED) {return ((GS_ECB *) presource->RCB_NextRCBWEL);}
...
G_INT32 gkm_ECB_list_ORDER(GS_ECB *pevent)
{
    ...
    if (pevent->ECBType == G_ECBType_MESSAGE_PRODUCE_DELIVERED) {return ((G_INT32) 1);}
...
G_INT32 gkm_Check_ECBs(G_INT32 partial)
{
    ...
    for (pevent = g_kcb.KCB_ROOT_ECBs; pevent != (GS_ECB *) 0; pevent = pevent->ECB_NEXT_ECBs)
	{
        ...
        if (partial == G_FALSE)
        {
            ...
            if (!((!antecedent) || ((pevent->ECB_AssocTCB->TCBState == G_TCBState_WAITING) || (pevent->ECB_AssocTCB->TCBState == G_TCBState_WAITING_COMPLETED)))) {printf("Invalid ISR waiting event type=%d, task state=%d, proc=%d", (int) pevent->ECBType, (int) pevent->ECB_AssocTCB->TCBState, (int) GRTOS_CMD_PRC_ID); while(1); return (G_FALSE);}
...
G_INT32 gkm_Check_TCBs(G_INT32 partial)
{
    ...
    for (ptcb = g_kcb.KCB_ROOT_TCBs; ptcb != (GS_TCB *) 0; ptcb = (GS_TCB *) ptcb->TCB_NEXT_TCBs) {
        ...
        if (!((!antecedent) || (ptcb->TCBCurrentPriority <= ptcb->TCB_NextTCB->TCBCurrentPriority))) {printf("Invalid TCBCurrentPriority=0x%llx, next=0x%llx", (unsigned long long) ptcb->TCBCurrentPriority, (unsigned long long) ptcb->TCB_NextTCB->TCBCurrentPriority); while(1); return (G_FALSE);}
...
G_INT32 gkm_Check_PCBAssocLCBs(G_INT32 partial)
{
    ...
    for (ppcb_assoc_lcb = g_kcb.KCB_ROOT_RDYs; ppcb_assoc_lcb != (struct gs_pcb_rdy_lcbl *) 0; ppcb_assoc_lcb = ppcb_assoc_lcb->gs_pcb_lcbl_nexts) {
        ...
        // Previous pcbassoclcb
        antecedent = ((ppcb_assoc_lcb->gs_pcb_rdy_lcbl_prev == (struct gs_pcb_rdy_lcbl *) 0));
        consequent = (g_kcb.G_PCBTbl[(ppcb_assoc_lcb->gs_pcb_assoc_pcb)-1].PCB_AssocLCB == ppcb_assoc_lcb);
        if (!(consequent || (!antecedent))) {printf("Invalid previous pcbassoclcb"); while(1); return (G_FALSE);}

        // Next pcbassoclcb
        antecedent = ((ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next != (struct gs_pcb_rdy_lcbl *) 0));
        if (!((!antecedent) || ((ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next->gs_pcb_rdy_lcbl_prev == ppcb_assoc_lcb) && (ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next->priority >= ppcb_assoc_lcb->priority)))) {printf("Invalid next pcbassoclcb"); while(1); return (G_FALSE);}

        // LCB previous of next
        antecedent = ((ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next != (struct gs_pcb_rdy_lcbl *) 0));
        if (!((!antecedent) || ((ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next->gs_pcb_rdy_lcbl_prev == ppcb_assoc_lcb) && (ppcb_assoc_lcb->gs_pcb_rdy_lcbl_next->priority >= ppcb_assoc_lcb->priority)))) {printf("Invalid previous pcbassoclcb of next"); while(1); return (G_FALSE);}
...
G_INT32 gkm_Check_LCBs(G_INT32 partial)
{
    ...
    for (plcb = g_kcb.KCB_ROOT_LCBs; plcb != (GS_LCB *) 0; plcb = plcb->LCB_NEXT_LCBs) {
        ...
        // LCB exclusion
        antecedent = (1);
        consequent = (plcb->LCBExclusion >= plcb->LCBCurrentRunning);
        if (!((!antecedent) || consequent)) {printf("Invalid LCB exclusion"); while(1); return (G_FALSE);}
...
G_INT32 gkm_Check_PCBs(G_INT32 partial)
{
    ...
    for (i = 0;  i < G_NUMBER_OF_PCB;  i++) {
        ...
        // Next free PCB valid
        antecedent = (g_kcb.G_PCBTbl[i].PCB_NextPCB != (struct gs_pcb *) 0);
        consequent = ((gkm_PCB_IsValid(g_kcb.G_PCBTbl[i].PCB_NextPCB) == G_TRUE) && (g_kcb.G_PCBTbl[i].PCB_NextPCB->PCBState == GS_PCBState_FREE));
        if (!((!antecedent) || consequent)) {printf("Error Next free PCB valid"); while(1); return (G_FALSE);}

        // Previous PCB valid
        antecedent = (g_kcb.G_PCBTbl[i].PCB_PrevPCB != (struct gs_pcb *) 0);
        consequent = ((gkm_PCB_IsValid(g_kcb.G_PCBTbl[i].PCB_PrevPCB) == G_TRUE) && (g_kcb.G_PCBTbl[i].PCB_PrevPCB->PCBState == GS_PCBState_FREE));
        if (!((!antecedent) || consequent)) {printf("Error Previous free PCB valid"); while(1); return (G_FALSE);}
...
{
    if (gk_trg_monitor(G_FALSE) == G_FALSE) {return (G_FALSE);}
...
G_INT32 gkm_Check_list_LCBL_IsValid(int cycles)
{
	...
	if ((count >= cycles))
    {
        count = 0;
        // Check trigger
       if (gk_trg_monitor(G_TRUE) == G_FALSE) {return (G_FALSE);}
