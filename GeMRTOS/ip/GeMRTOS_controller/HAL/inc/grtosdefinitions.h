...
#define gm_ProcessorInterruptDisable      \
		...
		/* 24-10-2024 do { \
			IOWR(GRTOS_DRIVER_GRTOS_BASE, ADDR_PRC_CRTCL_SECTION_START, gm_ProcessorId); \
		} while (((unsigned int) IORD(GRTOS_DRIVER_GRTOS_BASE, ADDR_PRC_CRTCL_SECTION_START) & (unsigned int) PRC_MASK[gm_ProcessorId-1]) == (unsigned int) 0); \
		enum pcbtype GeMRTOS_prev_PCBType = g_kcb.G_PCBTbl[gm_ProcessorId-1].PCBType; \
		g_kcb.G_PCBTbl[gm_ProcessorId-1].PCBType  = (int) GS_PCBType_UNAVAILABLE; \
		gk_SetLowestProcessor(); \
		gm_GeMRTOSCriticalSectionExit; \ 
		
		IOWR(GRTOS_DRIVER_GRTOS_BASE, ADDR_PRC_CRTCL_SECTION_END, gm_ProcessorId); \
		g_kcb.G_PCBTbl[gm_ProcessorId-1].PCBType = GeMRTOS_prev_PCBType; \
		*/
