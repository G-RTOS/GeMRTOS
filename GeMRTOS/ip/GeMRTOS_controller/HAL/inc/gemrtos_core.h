...
enum ecbtype {
	 ...
	 *  \ingroup ECBType
	 */
	G_ECBType_MESSAGE_PRODUCE_DELIVERED = 35,
...
struct gs_pcb {
	...
	HANDLE CurrentTaskThreadHandle;
#endif
};
...
struct gs_kcb {
    ...
    struct g_rgb *KCB_NextRCBTRGL[G_NUMBER_OF_IRQ_TCB]; ///< \brief Pointers to the RCB of triggers
...
#ifndef __niosX_arch__
    unsigned int fake_GeMRTOS_controller[G_NUMBER_OF_GRTOS_REGISTERS];
    G_INT32 fake_grtos_cmd_prc_id;
    HANDLE MainThreadHandle;
    CONDITION_VARIABLE kcb_critical_pthread_condition;
    CRITICAL_SECTION kcb_critical_pthread_mutex;
    CRITICAL_SECTION R_PRC_RST_mutex;

    pthread_mutex_t R_IRQ_PND_mutex;
    pthread_cond_t R_IRQ_PND_condition;
    HANDLE GeM_controlertid;
	CONDITION_VARIABLE GeMRTOS_RESERVE_MUTEX_condition;
	CRITICAL_SECTION GeMRTOS_MUTEX_section;
	CONDITION_VARIABLE R_PRC_RST_condition;
#endif
 ...
 */
enum tcbtype {
	/** \brief The <b>G_TCBType_UCOS</b> task type makes the task code to be executed just once. The task must be released once again for another execution if the task code does not contain an infinite loop. Initialization tasks may be implemented as a <b>G_TCBType_UCOS</b> task type without infinite loop in the task code. G_TCBType_UCOS tasks are often implemented as an infinite loop to keep them running. When a task with an infinite loop is executed, it will take as much processor time as possible.  It is possible to use different strategies to prevent one or many system tasks from being overly greedy about processor time and starving the others:
	...
	/** \brief The <b>G_TCBType_ISR</b> task type determines that the task is associated with a trigger resource. The G_TCBType_ISR type of the task should be set after the task is created using the <em>gu_trigger_register_task</em> function.
	 *  \ingroup TCBType
	...
	/** \brief Task type UNDEFIEND. When not a specific type is given to the task    
	 *  \ingroup TCBType
	 *  \remark G_TCBType_UNDEFINED
	 */
	G_TCBType_UNDEFINED = 7
};
/** @} */
...
struct gs_tcb 
{
    ...
    G_INT64  TCBInherPriority;              ///< \brief Priority Inhered by granted resource
    G_INT64  TCBRunPriority;                ///< \brief Priority when it is executed
    G_INT64  TCBCurrentPriority;            ///< \brief Current priority of the task according it is ready or running
	G_INT64  TCBPeriod;                     ///< \brief Period of the task
	G_INT64  TCBDeadline;                   ///< \brief Deadline of the task
	...
	G_INT32 TCB_Abort_w_Deadline;             ///< \brief True if task should be aborted when deadline, false otherwise
...
/**
 *  \brief gemrtos_jtag_uart_info  Structure to hold JTAG-UART informetio of GeMRTOS IP component
 *  \details The gemrtos_jtag_uart_info structure has all the fields to hold the JTAG-UART parameters.
 *  In system.h file, GRTOS_DRIVER_JTAG_ARRAY defines the structure array to hold all the information 
 *  of JTAG-UART components.
 */
struct gemrtos_jtag_uart_info {
	int JTAG_ID;     ///< \brief It is the ID of the JTAG-UART (generally, 0 for STDIO). It is like COMX.
	                   ///< \brief If no JTAG-UART then JTAG_ID is equal to -1
	void * JTAG_UART_BASE;
	unsigned int JTAG_UART_IRQ;
	unsigned int JTAG_UART_IRQ_INTERRUPT_CONTROLLER_ID;
	char * JTAG_UART_NAME;
	unsigned int JTAG_UART_READ_DEPTH;
	unsigned int JTAG_UART_READ_THRESHOLD;
	unsigned int JTAG_UART_SPAN;
	char * JTAG_UART_TYPE;
	unsigned int JTAG_UART_WRITE_DEPTH;
	unsigned int JTAG_UART_WRITE_THRESHOLD;
};
...
G_INT32   gk_LCB_CheckInvertion(void) __attribute__((optimize("-Os")));
G_INT32    gk_SetLowestProcessor(void) __attribute__((optimize("-Os")));
G_INT32    gk_SetNextTimeProcessor(void);
GS_TCB *gk_PCB_GetNextTCB(void) __attribute__((optimize("-Os")));
...
G_INT32 gkm_RCB_IsValid(void *prcb) __attribute__((section ("onchipmem"), optimize("-Os")));
...
G_INT32  gk_ECBTL_Link (GS_ECB *pevent) __attribute__((section ("onchipmem"), optimize("-Os")));
...
G_INT32    gk_LCBFPL_Link(G_INT32 processorID) __attribute__((optimize("-Os")));
G_INT32    gk_LCBFPL_Unlink(G_INT32 processorID) __attribute__((optimize("-Os")));
...
GS_ECB *gk_RCBWEL_Link(G_RCB *presource, GS_ECB *pevent) __attribute__((section ("onchipmem"), optimize("-Os")));
...
G_INT32    gk_TCB_Unlink(GS_TCB *ptcb)  __attribute__((optimize("-Os")));
...
G_INT32    gk_TCBRUNL_Unlink(GS_TCB *ptcb) __attribute__((optimize("-Os")));

G_INT32    gk_TCBWL_Link(GS_TCB *ptcb, unsigned int state);
G_INT32    gk_TCBWL_Unlink(GS_TCB *ptcb) __attribute__((optimize("-Os")));
...
void gk_ENTRY_IRQ_HANDLER (void) __attribute__((section ("onchipmem"), optimize("-Os")));
...
void gk_IRQ_HANDLER (void) __attribute__((section ("onchipmem"), optimize("-Os")));                    
G_INT32 gk_TASK_STK_Init(GS_TCB *ptcb)  __attribute__((optimize("-Os")));;
...
void  gk_KERNEL_TASK_COMPLETE(void) __attribute__((section ("onchipmem"), optimize("-Os")));
void gk_TCB_GetPendingSCB(void);
void gk_CheckASM(unsigned int data);
...
void gk_WAIT_IDLE_TASK(void) __attribute__ ((section ("onchipmem")));
...
G_INT32 gu_SetTaskAbortwhenDeadline(struct gs_tcb *ptcb, unsigned int abort_when_deadline);
...
// void *gu_GetTask(void *TaskCode, void *p_arg, char *task_description);
