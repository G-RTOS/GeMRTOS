...
struct gs_tcb 
{
    ...
    // 19-06-2024 G_INT64  TCBCurrentPriority;            ///< \brief Current priority of the task according it is ready or running
	...
	// 05-07-2024 G_INT32 TCB_Abort_w_Deadline;             ///< \brief True if task should be aborted when deadline, false otherwise
...
struct gemrtos_jtag_uart_info {
	G_INT32 JTAG_ID;     ///< \brief It is the ID of the JTAG-UART (generally, 0 for STDIO). It is like COMX.
	                   ///< \brief If no JTAG-UART then JTAG_ID is equal to -1
	enum uart_type UART_TYPE;
	void * JTAG_UART_BASE;
	G_INT32 JTAG_UART_IRQ;
	G_INT32 JTAG_UART_IRQ_INTERRUPT_CONTROLLER_ID;
	char * JTAG_UART_NAME;
	G_INT32 JTAG_UART_READ_DEPTH;
	G_INT32 JTAG_UART_READ_THRESHOLD;
	G_INT32 JTAG_UART_SPAN;
	char * JTAG_UART_TYPE;
	G_INT32 JTAG_UART_WRITE_DEPTH;
	G_INT32 JTAG_UART_WRITE_THRESHOLD;
	G_RCB * JTAG_UART_OUTPUT_QUEUE;
	G_RCB * JTAG_UART_INPUT_QUEUE;
	G_RCB * JTAG_UART_OUTPUT_TRIGGER;
	G_RCB * JTAG_UART_INPUT_TRIGGER;
};
...
extern const struct gemrtos_jtag_uart_info G_UART_INFO[];
...
// 05-07-2024 G_INT32 gu_SetTaskAbortwhenDeadline(struct gs_tcb *ptcb, unsigned int abort_when_deadline);
