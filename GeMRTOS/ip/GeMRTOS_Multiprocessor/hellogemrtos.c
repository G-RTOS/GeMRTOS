...
void task_philosopher(void* pdata)
{
    ...
    PRINT_ASSERT((i < NUMBER_PHILOSOPHERS),"ERROR In i, i=%u, arg=%u, %s\n", i, (unsigned int) ptcb->TCB_TaskArg, ptcb->TCB_description);

    PRINT_ASSERT((gkm_RCB_IsValid(forks[left_fork(i)]) == G_TRUE),"ERROR Left fork not valid, value = %p, i=%u\n", (void *) forks[left_fork(i)], i);
    PRINT_ASSERT((gkm_RCB_IsValid(forks[right_fork(i)]) == G_TRUE),"ERROR Right fork not valid, value = %p, i=%u\n", (void *) forks[right_fork(i)], i);
	
	/* get the semaphores in order */
	if (( i & 1) == 0) {  // philosopher is even
		gu_sem_wait(forks[left_fork(i)], (int) G_TRUE);
		gu_sem_wait(forks[right_fork(i)], (int) G_TRUE);
	} else {              // philosopher is odd
		gu_sem_wait(forks[right_fork(i)], (int) G_TRUE);
		gu_sem_wait(forks[left_fork(i)], (int) G_TRUE);
	}

	/* change the leds value             */
    leds = leds + 1; // ^ (unsigned int) ptcb; // xor with ptcb
	/* write the value in leds variable to the GeMRTOS output port  */
	gu_write_outputs(leds);
	
	gu_printf("%s is eating in processor %u\n", ptcb->TCB_description, (unsigned int) GRTOS_CMD_PRC_ID);
	gu_queue_printf(pqueue_out,"%s is eating in processor %d\n", ptcb->TCB_description, (int) GRTOS_CMD_PRC_ID);	

	/* release the semaphore in order */
	gu_sem_post((G_RCB *) forks[left_fork(i)]);
	gu_sem_post((G_RCB *) forks[right_fork(i)]);	
}
...
void GRTOS_IRQ_READ_SERVER(void *pdata)
{
    ...
    // 02-07-2024 void *mem = malloc(jtag_uart_0_info->buffer_length_in);
	// 02-07-2024 PRINT_ASSERT(((unsigned int) mem % 4 == 0), "ERROR memory is not alligned");
	// 02-07-2024 PRINT_ASSERT(((unsigned int) mem != 0), "ERROR no memory available");
    // 02-07-2024 char *buffer_msg = (char *) mem;
	...
	while (1) {
		...
		while ((uart_data = IORD_ALTERA_AVALON_JTAG_UART_DATA(JTAG_UART_0_BASE)) & ALTERA_AVALON_JTAG_UART_DATA_RVALID_MSK) {
			...
			index_msg++;
			// GU_FPRINTF(stderr, "index_msg= %d, character = %c\n", index_msg, data);
			...
			if ((data == '\r') || (data == '\n') || (index_msg >= jtag_uart_0_info->buffer_length_in - 2)) {
				...
				GU_FPRINTF(stderr, "\nINPUT FROM JTAG-UART %d= %s END\n", index_msg, buffer_msg);
...
void jtag_irq_manager(int pdata)
{
...
#ifdef __niosX_arch__
	volatile G_INT32 pendings = GRTOS_CMD_IRQ_RQS;
    ...
    if ((write_interrupt == (G_INT32) 1) && (jtag_uart_0_out->trigger.TRG_Enabled == G_TRUE)) {
		...
		gu_trigger_release ((int) jtag_uart_0_out);
	}
    ...
    if ((read_interrupt == (G_INT32) 1) && (jtag_uart_0_in->trigger.TRG_Enabled == G_TRUE)) {
		...
		// gu_fprintf("\nread interrupt= %s, pending=%x, ctrl_reg=%x\n", __FUNCTION__, pendings, uart_ctrl_register);
...
int main(void)
{
	...
	{
		/* Create a new Task */
		ptcb = gu_GetTask((void *) task_philosopher,    ///< Pointer to the task code
		...
		if (ptcb != (void *) 0) { /* Task was created successfully */
			...
			gu_SetTaskReadyPriority(ptcb, (G_INT64) i+1);
			gu_SetTaskRunPriority(ptcb,(G_INT64) i+1);

			/* Set the period equal to the task index plus one in seconds */
			gu_SetTaskPeriod(ptcb, 0, 0, 0, (i+1) * 100);
			
			/* Set the task with abort when deadline */
			// 05-07-2024 gu_SetTaskAbortwhenDeadline(ptcb, 0);
			
			/* Start execution of task with an offset equal to five seconds */
			gu_StartTaskwithOffset(ptcb, 0, 0, 5, 0);
			gu_signal_create(G_SCBType_TCB_ABORTED, 0, (void *) ptcb, (void *) sig_aborted_task_generic, (void *) i);  /// Abort when deadline
		}
		else {
	...
	if (ptcb != (void *) 0)
	{
		...
		gu_SetTaskReadyPriority(ptcb, (G_INT64) 0);
		gu_SetTaskRunPriority(ptcb, (G_INT64) 0);
