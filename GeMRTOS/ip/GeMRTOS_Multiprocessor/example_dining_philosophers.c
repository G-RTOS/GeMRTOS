...
#define MAX_PHILOSOPHERS_ALLOWED (NUMBER_PHILOSOPHERS-1)
...
void task_philosopher(void* pdata)
{
    ...
    PRINT_ASSERT((i < NUMBER_PHILOSOPHERS),"ERROR In i, i=%u, %s\n", i, ptcb->TCB_description);
	...
	if (( i & 1) == 0) {  // philosopher is even
		int sem_left  = (int) gu_sem_wait(forks[left_fork(i)], (int) G_TRUE);
		int sem_right = (int) gu_sem_wait(forks[right_fork(i)], (int) G_TRUE);
    ...
    leds = leds ^ (unsigned int) ptcb; // xor with ptcb
...
int main(void)
{
	...
	for (int i = 0; i < NUMBER_PHILOSOPHERS; i++)
	{
		...
		if (ptcb != (void *) 0) { /* Task was created successfully */
			...
			gu_SetTaskPeriod(ptcb, 0, 0, 1, i * 100);
